...
const Dashboard = () => {
  ...
  const calculatePercentage = (currentData: any[]) => {
    ...
    const recent = currentData.filter(item => item.created && new Date(item.created).getTime() > sixHoursAgo).length;
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (distance < 0) {
        const newTarget = (new Date().getTime() + (6 * 60 * 60 * 1000)).toString();
  ...
  const totalTransaksi = dataPenjualan.reduce((sum, item) => sum + (item.total_amount || 0), 0);
  ...
  const conversionRate = totalTraffic > 0 ? ((totalTransaksiCount / totalTraffic) * 100).toFixed(1) : "0.0";
  const engagementRate = totalTraffic > 0 ? ((totalLikes / totalTraffic) * 100).toFixed(1) : "0.0";
    ...
    <div className="space-y-6">
      ...
      >
        <div className="bg-white text-black rounded-xl shadow-lg p-6 hover:-translate-y-1 transition-transform duration-300">
          ...
          <p className={`text-sm mt-2 font-bold ${indArtikel.color}`}>{indArtikel.icon} {indArtikel.val}%</p>
        </div>

        <div className="bg-white text-black rounded-xl shadow-lg p-6 hover:-translate-y-1 transition-transform duration-300">
          <p className="text-md opacity-70">Traffic</p>
          <h2 className="text-4xl font-bold mt-2">{dataTraffic.length}</h2>
          <p className={`text-sm mt-2 font-bold ${indTraffic.color}`}>{indTraffic.icon} {indTraffic.val}%</p>
        </div>
        ...
        <div className="bg-white text-black rounded-xl shadow-lg p-6 hover:-translate-y-1 transition-transform duration-300">
          ...
          <h2 className="text-2xl md:text-2xl lg:text-xl xl:text-2xl font-bold mt-2 truncate">
            {totalTransaksi.toLocaleString("id-ID", { style: "currency", currency: "IDR" })}
          </h2>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
        <div className="lg:col-span-2 bg-slate-800 rounded-xl shadow-lg p-6">
          <h2 className="text-xl font-bold mb-6 text-white">Statistik Pendapatan per Bulan</h2>
          <ResponsiveContainer width="100%" height={350}>
            <LineChart data={dataChartPenjualan}>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151" vertical={false} />
              <XAxis dataKey="bulan" stroke="#9ca3af" />
              <YAxis stroke="#9ca3af" />
              <Tooltip contentStyle={{ backgroundColor: '#1f2937', borderColor: '#374151', color: '#fff', borderRadius: '8px' }} formatter={(value: any) => `Rp ${Number(value).toLocaleString("id-ID")}`} />
              <Legend wrapperStyle={{ color: '#9ca3af' }} />
              <Line type="monotone" dataKey="penjualan" stroke="#3b82f6" strokeWidth={4} dot={{ r: 4, fill: '#3b82f6' }} activeDot={{ r: 8 }} name="Total Pendapatan" />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="lg:col-span-1 bg-white rounded-xl shadow-lg p-6">
          ...
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={dataBlog} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e5e7eb" />
              <XAxis dataKey="artikel" tick={{ fontSize: 12, fill: '#6b7280' }} tickFormatter={(val) => val.length > 8 ? val.substring(0, 8) + '...' : val} />
              <YAxis tick={{ fontSize: 12, fill: '#6b7280' }} />
              <Tooltip contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
              <Legend />
              <Bar dataKey="likes" fill="#f97316" name="Jumlah Like" radius={[4, 4, 0, 0]} barSize={40} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
