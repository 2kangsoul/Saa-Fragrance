...
export default function AdminManagerModal({ isOpen, onClose }: AdminManagerModalProps) {
  ...
  const handleMakeAdmin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Berhasil menjadikan ${existingFormData.email} sebagai admin!`);
          ...
          <div className="bg-gray-50 p-6 rounded-xl border border-gray-100 h-max">
            ...
            <div className="flex bg-gray-200 p-1 rounded-lg mb-6">
              <button
                ...
                className={`flex-1 text-xs font-bold py-2 rounded-md transition-colors ${
                  activeForm === "new" ? "bg-white text-gray-900 shadow" : "text-gray-500 hover:text-gray-700"
                }`}
              >
                Tambah Admin Baru
              </button>
              <button
                ...
                className={`flex-1 text-xs font-bold py-2 rounded-md transition-colors flex items-center justify-center gap-1 ${
                  activeForm === "existing" ? "bg-white text-gray-900 shadow" : "text-gray-500 hover:text-gray-700"
                }`}
              >
                Jadikan Admin
                {/* Tanda Panah */}
            ...
            {activeForm === "new" ? (
              ...
              <form onSubmit={handleMakeAdmin} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Email Pengguna Aktif</label>
                  <input
                    type="email"
                    value={existingFormData.email}
                    onChange={(e) => setExistingFormData({ ...existingFormData, email: e.target.value })}
                    placeholder="email.user@gmail.com"
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:outline-none text-sm"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Durasi Admin</label>
                  <select
                    value={existingFormData.duration}
                    onChange={(e) => setExistingFormData({ ...existingFormData, duration: e.target.value })}
